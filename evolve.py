...
# Function to calculate the score of a painting
def score(x: Painting) -> float:
    current_score = x.image_diff(x.target_image)
    print(".", end='', flush=True)
    return current_score

# Function to select the best individual and a random individual for breeding
...
if __name__ == "__main__":
    ...
    # Evolution stages with fine-tuned parameters

    early_evo = (Evolution()
                 .survive(fraction=0.05)
                 .breed(parent_picker=pick_random, combiner=mate, population_size=population_size)
                 .mutate(mutate_function=mutate_painting, rate=0.08, swap=0.6)
                 .evaluate(lazy=False)
    ...
    # Evolve the population with the defined evolution stages and iterations
